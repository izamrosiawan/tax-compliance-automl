...
def generate_all_rigorous_visuals():
    ...
    plt.axhline(0.5, color="red", linestyle="--", linewidth=1, label="Garis Dasar Acak (AUC = 0,50)")
    plt.axhline(0.7682, color="navy", linestyle=":", linewidth=1, label="Model XGBoost Uji Penuh (0,7682)")
    plt.ylim(0.4, 1.0)
    plt.ylabel("Holdout ROC-AUC", fontsize=9)
    plt.xlabel("Provinsi Uji (Unseen Holdout)", fontsize=9)
    for i, v in enumerate(geo_eval["ROC_AUC"]):
        plt.text(i, v - 0.04, f"{v:.4f}", ha="center", color="white", fontweight="bold", fontsize=8)
    plt.title("Geographical Generalization: Cross-Province Holdout Evaluation (R=5)", fontsize=10, pad=10)
