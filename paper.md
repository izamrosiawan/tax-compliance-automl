# Kerangka Benchmark Simulasi Non-Sirkular untuk Pemeringkatan Risiko Kepatuhan Pajak Pedagang Daring melalui Integrasi Data Transaksi, Logistik, dan Indikator Regional Terkalibrasi BPS
...
Dalam kerangka *Compliance Risk Management* (CRM), otoritas pajak mengelompokkan wajib pajak ke dalam kuadran risiko guna menetapkan prioritas tindakan (Direktorat Jenderal Pajak, 2023). Karena data individual SPT wajib pajak dilindungi oleh asas kerahasiaan jabatan (Pasal 34 UU KUP), riset komputasional memerlukan tolok ukur simulasi sintetis (*synthetic simulation benchmark*) yang dirancang secara non-sirkular untuk menguji nilai informasi dari integrasi data pihak ketiga.

Penelitian ini bertujuan untuk menjawab tiga pertanyaan penelitian utama (*Research Questions*):
1. **(RQ1):** Seberapa besar kontribusi penambahan data transaksi digital dan logistik dibandingkan pelaporan SPT mandiri dalam memeringkat risiko kepatuhan wajib pajak pada lingkungan simulasi?
2. **(RQ2):** Apakah penambahan indikator makro regional terkalibrasi BPS memberikan peningkatan diskriminasi yang bermakna melampaui data transaksi mikro?
...
b. **Studi Ablasi Fitur Bertahap:** Mengukur kontribusi marginal dari data pelaporan mandiri, transaksi digital, logistik, dan data regional terkalibrasi BPS dengan interval kepercayaan 95% (*95% Confidence Interval*).
...
| $EComPct_p$ | Provinsi (Makro) | BPS Benchmark Provinsi $p \pm \mathcal{N}(0, 1.5^2)$ | Rujukan BPS (2024) Tabel 3.1; mencerminkan penetrasi usaha e-commerce regional ($24,32\% - 63,54\%$). | **Tidak** |
...
Dataset 5.000 observasi dibagi menjadi 80% data latih ($n=4.000$) dan 20% data uji independen ($n=1.000$) menggunakan pemisahan berstrata ($\text{seed}=42$). Standarisasi fitur (*StandardScaler*) di-fit secara eksklusif pada data latih dan ditransformasikan ke data uji guna mencegah kebocoran data (*data leakage*).

Optimasi hyperparameter dijalankan pada data latih menggunakan Optuna v3.6 dengan algoritma *Tree-structured Parzen Estimator* (TPE, 20 trial) di dalam validasi silang berstrata 5-Fold. Lingkungan eksperimen menggunakan Python 3.11, `scikit-learn` v1.4, `xgboost` v2.0, `lightgbm` v4.3, dan `shap` v0.45. Waktu komputasi total untuk seluruh pipeline (pembangkitan, training 4 model, TPE, validasi holdout berulang, dan kalkulasi SHAP) adalah 18,4 detik pada prosesor AMD Ryzen 7 / Intel Core i7 standar.

Estimasi interval kepercayaan 95% (95% CI) dihitung menggunakan metode *non-parametric percentile bootstrap* ($B=500$ iterasi) yang dijalankan secara eksklusif pada pasangan probabilitas prediksi dan label data uji dengan bobot model yang telah dibekukan (*fixed trained model weights*).
...
| **4. Model Penuh (+ 2 Indikator Makro BPS)** | **0,7684** | **0,5467** | **45,2%** | **2,26x** | Memberikan konteks makro wilayah (marginal $\Delta\text{AUC}=+0,0017$) |
...
Berdasarkan Tabel 4:
a. Pelaporan SPT mandiri semata memiliki daya pembeda yang sangat rendah (ROC-AUC $0,5641$).
b. Penambahan data transaksi perbankan dan logistik berkorelasi dengan lonjakan performa terbesar (ROC-AUC naik ke $0,7667$ dan PR-AUC $0,5455$, sekitar 2,18 kali garis dasar prevalensi).
c. Penambahan **dua indikator regional BPS yang diuji** memberikan peningkatan diskriminasi marginal ($\Delta\text{AUC} = +0,0017$). Pergeseran kecil pada Top-20% Yield dari 46,0% ke 45,2% disebabkan oleh sedikit kalibrasi ulang pada distribusi probabilitas prediksi di batas desil kedua, namun tetap mempertahankan faktor lift yang stabil ($2,26\times$).
