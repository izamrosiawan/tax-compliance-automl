...
| $Infra_p$ | Provinsi (Makro) | BPS Benchmark IP-TIK 2023 Provinsi $p \pm \mathcal{N}(0, 0.12^2)$ | Contextual Macro Feature ($X$) | **Tidak** |
...
*Catatan redundansi administratif:* Variabel $TaxPaid_i$ dipertahankan dalam model sebagai representasi nilai nominal pembayaran pajak yang secara administratif diturunkan secara deterministik dari nilai SPT ($SPT_i \times 0,005$) untuk mensimulasikan kelengkapan atribut basis data perpajakan standar. Karena bersifat deterministik terhadap SPT, $TaxPaid_i$ tidak diinterpretasikan sebagai sumber informasi independen baru dalam analisis kontribusi fitur.
...
Dengan memprioritaskan pemeriksaan pada **Top 20% kelompok berisiko tertinggi (Desil 1 dan 2)**, rentang tangkapan temuan lintas kombinasi model dan konfigurasi fitur yang dievaluasi berkisar antara **$45,2\%$ hingga $47,6\%$ dari total kasus positif yang disimulasikan** (menghasilkan faktor pengali *cumulative lift* sebesar **$2,26\times$ hingga $2,38\times$** dibandingkan pemilihan acak 20%).
